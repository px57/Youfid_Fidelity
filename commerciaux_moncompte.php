<html>
	...
	<?php
		...
		$logger->log('debug', 'commerciaux_moncompte', "in file", Logger::GRAN_MONTH);
		...
		//require_once("lib/commerciaux_register_marchand.php"); 
	?>
	...
	<div id="new_marchant_content" style="margin:10px 30px">
		<h2>Mon Compte</h2>
		<form id="new_merchant_form" name="new_merchant_form" method="post" action="lib/commerciaux_register_marchand.php">
			...
			<div class="new_merchant_up">
								...
								xhr.onreadystatechange = function() {
									if (xhr.readyState == 4 && (xhr.status == 200 || xhr.status == 0)) {
										if ((xhr.responseText) == "OK") 
											...
											location.replace("commerciaux_moncompte.php");
									}
								};
				...
				<div class="new_merchant_up_right left">
					<label>Email backoffice : </label><br /><input type="text" title="Email de gestion pour le backoffice" id="email_bo" name="email_bo" value='<?php echo($email_bo); ?>' onfocus="if (this.value == '<?php echo($email_bo); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo($email_bo); ?>';}" /><br />
					...
					<label>Logo URL : </label><br /><input type="text" onKeyUp="display_url_picture(this)" <?php if(!$change_logo)echo('disabled="disabled"'); ?> title="Url du logo" name="logourl" value='<?php echo($logo); ?>' onfocus="if (this.value == '<?php echo($logo); ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php echo($logo); ?>';}" /><br />
			...
			<div class="new_merchant_bottom">
				...
				<!-- Preview -->
				<div id ="divdisplay2"><img src=""/></div>
			...
			<input <?php if (isset($_SESSION['selector']) && $_SESSION['selector'] == "NEW")echo('style="margin-right: 50%"'); ?> type="checkbox" id="is_supermarchand" name="is_supermarchand" <?php if ($supermarchand == "1")echo("checked") ?>/>
			<input type="button" onclick="onClickFormMonCompte()"  value="<?php echo($button_value); ?>"></br><span style="color: red" id="check_form_result"></span>
		</form>
	...
	<?php
		...
		{
			$logger->log('debug', 'commerciaux_moncompte', "Label_selector::onChange!!!", Logger::GRAN_MONTH);
		}
	?>
