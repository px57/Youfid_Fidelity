  ...
  if(idDisplay != null) getHTMLElement(idDisplay).innerHTML = sz + "/" + max;
        ...
        $(document).ready(function()
        {
            ...
            $('#submit2').click(function() {
                ...
                $.ajax({
                    type: "POST",
                    url: "lib/send-promos.php",
                    data: $("#promoForm").serialize(),
                    success: function(resp) {
                        $('#submit2').removeAttr("disabled");
                        $(location).attr("href", "marchand_promos.php");
                    }
                });
            });
    ...
    <form action="lib/calc_promo2.php" method="post" id="promoForm">
        <div id="left">
            <br/> <br/>
<?php echo $used; ?> Promos restantes ce mois-ci !
            ...
            <textarea class="promotext" id="titre" cols="60" rows="1" name="titre" onKeyUp="checkNbCar(35,'titre','NBCAR',true);"><?php echo $titre; ?></textarea>
            <b id="NBCAR"></b>
            ...
            <textarea class="promotext" id="contenu" cols="60" rows="7" name="contenu" onKeyUp="checkNbCar(165,'contenu','NBCAR2',true);"><?php echo $contenu; ?></textarea>
            <b id="NBCAR2"></b>
...
<?php if ($rowMarchand['is_supermarchand'] != 1) { ?>
                ...
                <br/> <br/>
                <input type="checkbox" id="last_authent" name="last_authent" onclick="verif3()" <?php echo $last_authent; ?>> Clients dont la derniere authentification date de plus de  <select class="inputpromo" name="combien"><?php $i = 1;
    while ($i < 31) {
        if ($i == $combien) {
            ...
            echo "<option>$i";
                ...
                <select class="inputpromo"  name="chrono"><option <?php if ($chrono == 'Jours') {
        echo "selected";
    } ?>>Jours<option <?php if ($chrono == 'Semaines') {
                    echo "selected";
                } ?>>Semaines<option <?php if ($chrono == 'Mois') {
                    echo "selected";
                } ?>>Mois<option <?php if ($chrono == 'Années') {
                    echo "selected";
                } ?>>Années</select>
                <br/> <br/>
                ...
                <br/> <br/>
                <input type="checkbox" id="next_present" name="next_present" onclick="verif3()" <?php echo $next_present; ?>> Clients a une visite de <select name="next_cadeau"><?php
            while ($row = mysql_fetch_array($resultCadal)) {
            ...
            echo "<option> Tous les cadeaux";
            ?></select>
                <br/> <br/>
<?php
} else {
    ?>
                <input  type="checkbox" id="scanloc" name="scanloc" <?php echo $scanloc; ?> onclick="verif5()"> Clients scannés dans le <select name="localisation" class="inputpromo"><?php $i = 0;
    while (isset($LocArray2[$i])) {
        if ($LocArray2[$i] == $localisation) {
            echo "<option selected>$LocArray2[$i]";
        } else {
            echo "<option>$LocArray2[$i]";
        } $i++;
    } ?></select>
                <br/> <br/>
                <input  type="checkbox" id="time" name="time" value="time" <?php echo $time; ?> onclick="verif5()"> Dans le(s) dernier(e)(s) <select class="inputpromo" name="combien"><?php $i = 1;
    while ($i < 31) {
        if ($i == $combien) {
            echo "<option selected>$i";
        } else {
...
<?php } ?>
            <input type="checkbox" id="ALL" name="ALL" value="ALL" onclick="verif4()"  <?php if ($fidelity != 'checked' && $last_authent != 'checked' && $min_points != 'checked' && $ancient != 'checked' && $next_present != 'checked') {
    echo 'checked';
} ?>> Tous les clients
            <br/> <br/>
            <input type='submit' name='submit1' value='Calculer'> = <?php echo count($_SESSION['nb_client']); ?> <input type='button' id='submit2' name='submit2' value='Envoyer!' <?php if ($used <= 0) {
    echo 'disabled="disabled"';
} ?>>
            <br />
    </form>
    <FORM METHOD='LINK' ACTION='histo_promo.php'><INPUT TYPE='submit' VALUE='Historique'></FORM>   <FORM METHOD='LINK' ACTION='marchand_email_promo.php'><INPUT TYPE='submit' VALUE='E-mailing' <?php if ($used <= 0 || $rowMarchand['is_email_actif'] == '0') {
    echo 'disabled';
} ?>></FORM>
</div>
<div class="clear">
