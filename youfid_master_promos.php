<html>
	<?php
		require_once 'lib/push_message.php';
		...
		$_SESSION['selector_current_location'] = "youfid_master_promos.php";
		...
		$sqlMarchand = "SELECT * FROM marchand";
		...
		$sqlCategorie = "SELECT * FROM label WHERE type = 'categorie'";
		$result = mysql_query($sqlCategorie);
		$i = 0;
		while ($row = mysql_fetch_array($result)) {
			if (isset($row['nom'])) {
				$CatArray[$i] = $row['nom'];
	...
	<script type="text/javascript" language="javascript" src="./js/decompte.js"></script>
	
	<script>
	function pop(A) {
		window.alert('Promotion envoyée avec succès à ' + A + ' personnes.');
	}
	</script>
	...
	if ((document.getElementById("scanloc").checked == true 
		...
		document.getElementById("ALL").checked = false;
			...
			&& document.getElementById("domaine").checked == false
			&& document.getElementById("ALL").checked == false){
		document.getElementById("ALL").checked = true;
	}
	...
	function verif2(){ 
	 ...
	 if (document.getElementById("ALL").checked == true)	 {
		document.getElementById("scanloc").checked = false;
		document.getElementById("time").checked = false;
		document.getElementById("domaine").checked = false;
	}
	else if (document.getElementById("scanloc").checked == false 
	...
	{
	  var content = getHTMLElement(idText).value;
	  var sz = countBits(content);
	  if( sz > max )
	  {
	    sz = max;
	    if( lAlert )
	    {
	    	window.alert("Merci de ne pas dépasser " + max + " caractères !");
	    	do
	    	{
	    		content = content.slice(0,-1);
	    	}while(countBits(content) > max);
	    	
	    	document.getElementById(idText).value = content;
	    }
	  }
...
/*	$arrayRem['titre'] = $_POST['titre'];
	...
	$domaine = isset($arrayRem['domaine']) &&  $arrayRem['domaine'] ? 'checked' : '';
	$domaines = isset($arrayRem['domaines']) ? $arrayRem['domaines'] : '';
	?>
	<form action="lib/calc_promo.php" method="post" accept-charset="UTF-8">
		<div class="left" style="margin-left: 80px">
			<br/>
			<br/> <br/> <br/>
			Titre (max. 35 carac.) :
			<br/> <br/>
			<textarea class="promotext" cols="60" rows="1" name="titre" id="titre0" onKeyUp="checkNbCar(35,'titre0','NBCAR0',true);" ><?php echo $titre; ?></textarea>
			<b id="NBCAR0"></b>
			<br/> <br/> <br/>
			Contenu (max. 165 carac.) :
			<br/> <br/>
			<textarea class="promotext" cols="60" rows="7" name="contenu" id="titre1" onKeyUp="checkNbCar(165,'titre1','NBCAR1',true);"><?php echo $contenu; ?></textarea>
			<b id="NBCAR1"></b>
			<br/> <br/> <br/>
			Du <input type="text" name="date_debut" class="time" value="<?php echo $date_debut; ?>" READONLY/> au <input type="text" name="date_fin" class="time" value="<?php echo $date_fin; ?>" READONLY/>
		</div>
		...
		<div class="right"  style="margin-right: 80px">
			...
			Envoyé aux :
			<br/> <br/>
			<input  type="checkbox" id="scanloc" name="scanloc" <?php echo $scanloc; ?> onclick="verif()"> Clients scannés dans le <select name="localisation" class="inputpromo"><?php $i = 0; while (isset($LocArray2[$i])) {if ($LocArray2[$i] == $localisation) {echo "<option selected>$LocArray2[$i]";} else {echo "<option>$LocArray2[$i]";} $i++;} ?></select>
			<br/> <br/>
			<input  type="checkbox" id="time" name="time" value="time" <?php echo $time; ?> onclick="verif()"> Dans le(s) dernier(e)(s) <select class="inputpromo" name="combien"><?php $i = 1; while ($i < 31) {if($i == $combien){echo "<option selected>$i";} else{echo "<option>$i";} $i++;} ?></select>
			<select class="inputpromo" name="chrono"><option>Jours<option>Semaines<option>Mois<option>Années</select>
			<br/> <br/>
			<input  type="checkbox" id="domaine" name="domaine" value="domaine" <?php echo $domaine; ?> onclick="verif()"> Dans le domaine <select name="domaines"><?php $i = 0; while (isset($CatArray[$i])) {if($CatArray[$i] == $domaines){echo "<option selected>$CatArray[$i]";}  else{echo "<option>$CatArray[$i]";} $i++;} ?></select>
			<br/> <br/>
			<input type="checkbox" id="ALL" name="ALL" value="ALL" onclick="verif2()"  <?php if ($domaine != 'checked' && $time != 'checked' && $scanloc != 'checked'){echo 'checked';} ?>> Tous les clients
			<br/> <br/>
			<input type='submit' name='submit1' value='Calculer'> = <?php echo count($_SESSION['nb_client']) ?> <input type='submit' name='submit2' value='Envoyer!'>
			<br />
			<input type='submit' name='email_before' value='E-mailing'>
			</form>
			<FORM METHOD='LINK' ACTION='histo_promo.php'><INPUT TYPE='submit' VALUE='Historique'></FORM>  
		</div>
		<div class="clear"></div>
	<?php }
		else {
	...
	<form action="lib/calc_promo2.php" method="post" >
		<div id="left">
			...
			Titre (max. 35 car.) :
			<br/> <br/>
			<textarea class="promotext" cols="60" rows="1" name="titre" id="titre" onKeyUp="checkNbCar(35,'titre','NBCAR',true);"><?php echo $titre; ?></textarea>
			<b id="NBCAR"></b>
			<br/> <br/> <br/>
			Contenu (max. 165 car.) :
			<br/> <br/>
			<textarea class="promotext" cols="60" rows="7" name="contenu" id="contenu" onKeyUp="checkNbCar(165,'contenu','NBCAR2',true);"><?php echo $contenu; ?></textarea>
			<b id="NBCAR2"></b>
			<br/> <br/>
			Du <input type="text" name="date_debut" class="time" value="<?php echo $date_debut; ?>" READONLY/> au <input type="text" name="date_fin" class="time" value="<?php echo $date_fin; ?>" READONLY/>
