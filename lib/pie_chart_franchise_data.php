<?php
	...
	if (isset($logger))
		$logger->log('debug', 'pie_chart_franchise_data', "inFILE", Logger::GRAN_MONTH);
	...
	if (isset($logger))
		$logger->log('debug', 'pie_chart_franchise_data', "Response::" . $json_result , Logger::GRAN_MONTH);
	...
	function sort_data()
	{
		...
		/// Sort des marchand sous la forme : marchand/franchise => nb_autent
		foreach ($id_array as $id_merchant => &$value )
		{
		    if ($merchant = get_merchant_with_label($id_merchant))
		    {
		    	if ($merchant['supermarchand_id'] == -1)
					$sort_array[$merchant['name']] = $value;
				else
				{
					if (array_key_exists($merchant['label'], $sort_array))
						$sort_array[$merchant['label']] += $value;
					else
						$sort_array[$merchant['label']] = $value;
				}
		    }
		...
		if (isset($logger))
			$logger->log('debug', 'pie_chart_franchise_data', "insort_data::return::" . json_encode($dataTable), Logger::GRAN_MONTH);
		
		//return($top_ten);
		return $dataTable;
	}
	
	function get_merchant_with_label($merchant_id)
	{
		global $logger, $tbl_marchands, $tbl_label;
		
		$merchant_query = "SELECT * FROM $tbl_marchands WHERE `id`='"
			. mysql_real_escape_string($merchant_id)
			. "'";
			
		$merchant_result = mysql_query($merchant_query);
		
		if ($merchant_result == FALSE)
			return FALSE;
		
		if ($merchant_row = mysql_fetch_array($merchant_result))
		{
			$label_query = "SELECT * FROM $tbl_label WHERE `id`='"
				. mysql_real_escape_string($merchant_row['label_id'])
				. "'";
				
			$label_result = mysql_query($label_query);
			
			if ($label_result == FALSE)
				return FALSE;
			
			if ($label_row = mysql_fetch_array($label_result))
			{
				$result = array();
				$result['name'] = $merchant_row['name'];
				$result['is_supermarchand'] = $merchant_row['is_supermarchand'];
				$result['supermarchand_id'] = $merchant_row['supermarchand_id'];
				$result['label'] = $label_row['nom'];
