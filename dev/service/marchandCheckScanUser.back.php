...
///// SECTION: This section helps to mitigate multi concurrent requests
$semKey = ftok(__FILE__, 'R');
$sem = sem_get($semKey, 1);
sem_acquire($sem);

if(apc_exists($jsonArray->qr_code)) {
  exit();
}

apc_add($jsonArray->qr_code, true, 5);
sem_release($sem);
///// END SECTION
...
if ($error != TRUE && $process == true && ((isset($jsonArray->qr_code) || isset($jsonArray->usr_id)) && isset($jsonArray->merchant_id) && $maxChecked == 0)) {
  ...
  if ($usr_id == '0') {
    ...
    if ($totalScan >= $maxScan) {
      ...
      } /*else if(isMobileUserBolcked($rowMarchand["id"], $rowUser["id"]) || 
        ($rowMarchand["supermarchand_id"] == 699 && !fitOtacosSecurityRules(699, $rowMarchand["id"], $rowUser["id"]))) {
        $error    = true;
...
function fitOtacosSecurityRules($superMerchantId, $marchandId, $mobileUserId) {
  if(intval(date('H')) < 10) {
    blockMobileUser($mobileUserId, $marchandId);
    return false;
  }
  ...
  // Check if has 10 scans in a month
  $query = "SELECT COUNT(t.id) nbTrans
            FROM `transaction` t
            INNER JOIN marchand m ON m.id = t.marchand_id
            WHERE m.supermarchand_id = " . intval($superMerchantId) . "
            AND t.mobileuser_id = " . intval($mobileUserId) . "
            AND t.`transaction_date` > DATE_SUB( NOW( ) , INTERVAL 31 DAY)";
  $queryRes = mysql_query($query);
...
function checkCountinousDates($datesArray) {
  if(count($datesArray)) {
...
function blockMobileUser($mobileUserId, $merchandId) {
  mysql_query("UPDATE marchand_has_mobileuser SET block_date = NOW()
               WHERE mobileuser_id = " . intval($mobileUserId) . "
               AND marchand_id = " . intval($merchandId));
}

function isMobileUserBolcked($mobileUserId, $merchandId) {
  $queryRes = mysql_query("SELECT COUNT(id) nb FROM marchand_has_mobileuser
    WHERE mobileuser_id = " . intval($mobileUserId) . " AND marchand_id = " . intval($merchandId) . " AND block_date IS NOT NULL");
