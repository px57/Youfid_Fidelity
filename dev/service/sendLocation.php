<?php
	...
	function subscribe_user_to_sm($merchant_id, $user_id)
	{
		...
		$query = "INSERT DELAYED INTO $merchant_has_mbuser SET `marchand_id`='"
	...
	function update_last_notif($merchant_id, $mbu_id)
	{
		...
		$query = "UPDATE LOW_PRIORITY marchand_has_mobileuser SET `last_notif`=current_timestamp() WHERE `mobileuser_id`='"
		...
		$query = "INSERT DELAYED INTO histo_push SET `marchand_id`='"
			. mysql_real_escape_string($merchant_id)
			. "', `mobileuser_id`='"
			. mysql_real_escape_string($mbu_id)
	...
	function create_user_message($merchant_id, $titre, $message)
	{
		...
		$query = "INSERT DELAYED INTO message SET `marchand_id`='"
			. mysql_real_escape_string($merchant_id)
	...
	if (isset($jsonArray->usr_id) && isset($jsonArray->longitude) && isset($jsonArray->latitude) && isset($jsonArray->locationtime))
	{
		...
		$sqlSetLocation = "UPDATE LOW_PRIORITY $tbl_name SET `lattitude`='"
		...
		exec('mysql -u ' . $username . ' -p' . $password . ' -D ' . $db_name . ' -e "' . $sqlSetLocation . '" > /dev/null 2>/dev/null &');
/*
...
*/
		...
		while ($marchand = mysql_fetch_array($resultGetProxyMerchants))
		{
			...
			if($num==0)
			{
				$query = "INSERT DELAYED INTO marchand_has_mobileuser SET `marchand_id`='"
					. $marchand['id']
					. "', `mobileuser_id`='"
					. $mobile_user['id']
					. "', `nb_use`='0'";
...
////////////////////////////
					...
					// 1.a) Si le user a déjà reçu une promo ciblée valide de la part du marchand:
...
////////////////////////////////
						...
						if ($id_msg = create_user_message($marchand['id'], $old_promo['titre'], $old_promo['message']))
						{
							...
							$queryAdd = "INSERT DELAYED INTO message_has_mobileuser SET `mobileuser_id`='"
					...
					// 1.b) Si le user n'a pas de promo ciblée valide de la part du marchand:
					else
					{
////////////////////////////////
						doLog("</br>>>>Le user n'a jamais recu de promo ciblee de la part du marchand");

						// 1.b.i. Si le user est déjà affilié au marchand:
						if($marchand_has_mobileuser['nb_use'] > 0)
							doLog("</br>>>>Mais le user est deja affilie au marchand");
					...
					////////////
							...
							if ($toSend == 1 && $id_msg = create_user_message($marchand['id'], $message['message'], $message['detail']))
							{
								...
								$query = "INSERT DELAYED INTO message_has_mobileuser SET `mobileuser_id`='"
								. mysql_real_escape_string($mobile_user['id'])
								. "', `date_creation`=Now(), `message_id`='"
								. mysql_real_escape_string($id_msg)
								. "'";
...
////////////////////////
			...
			// 2. Si le user n'a jamais reçu de push géolocalisé de la part de ce marchand
			else
			{
////////////////////////
				doLog(" n'a jamais recu de push geolocalise de sa part");

				// 2.a) Si le user est déjà affilié au marchand:
				if($marchand_has_mobileuser['nb_use'] > 0)
					doLog("</br>>>>Mais le user est deja affilie au marchand");

				// 2.b) Si le user ne l'est pas
				else
				{
