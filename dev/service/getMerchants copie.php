<?php
	...
	function getMerchantsList($jsonArray)
	{
		...
		if(isset($jsonArray->supermarchand_id))
			$query = "SELECT * FROM $tbl_merchant WHERE supermarchand_id=" . intval($supermarchand_id) . " AND is_active ='1'";
	...
	function getMerchants($jsonArray)
	{
		...
		if ($have_point == FALSE) {
			if($have_location)
				$res = doGetMerchantWithoutPoints($user, $jsonArray->latitude, $jsonArray->longitude, $jsonArray->offset, $jsonArray->nb_merchants, $youfid_access->wsAccess);
			else
				$res = doGetMerchantWithoutPoints($user, 0, 0, $jsonArray->offset, $jsonArray->nb_merchants, $youfid_access->wsAccess);
		} else {
			if($have_location)
				$res = doGetMerchantWithPoints($user, $jsonArray->latitude, $jsonArray->longitude, $jsonArray->offset, $jsonArray->nb_merchants, $youfid_access->wsAccess);
			else
				$res = doGetMerchantWithPoints($user, 0, 0, $jsonArray->offset, $jsonArray->nb_merchants, $youfid_access->wsAccess);
		}
	...
	*/
	...
	/// Cas ou HAVE_POINT == 1
	function doGetMerchantWithPoints($user, $latitude, $longitude, $offset, $nb_rows, $wsAccess)
	{
		...
		if($latitude != 0 && $longitude != 0)
			$sqlGetMerchants = "SELECT m.id, m.name, m.application_id, m.label_id, m.logo, m.address, m.zip_code, m.city, m.phone, m.latittude, m.longitude, m.is_active, m.supermarchand_id, m.is_supermarchand,
								...
								WHERE mm.mobileuser_id = '$uid' AND m.is_active = 1 AND m.is_supermarchand = 0 AND mm.nb_use > 0  " . ($superFilterQry !== null ? "AND " . $superFilterQry : ""  ) . "
	...
	/// Cas ou HAVE_POINT == 0
	function doGetMerchantWithoutPoints($user, $latitude, $longitude, $offset, $nb_rows, $wsAccess)
	{
		...
		if($latitude != 0 && $longitude != 0)
			...
			FROM (
				SELECT m.id, m.name, m.application_id, m.label_id, m.logo, m.address, m.zip_code, m.city, m.phone, m.latittude, m.longitude, m.is_active, m.supermarchand_id, m.is_supermarchand,
					   GeoDistDiff('km', m.latittude, m.longitude, $latitude, $longitude) AS distance
				FROM `marchand` m
				WHERE m.is_active = 1 AND m.is_supermarchand = 0 " . ($superFilterQry !== null ? "AND " . $superFilterQry : ""  ) . "
				ORDER BY distance ASC
				LIMIT $offset, $nb_rows
			) AS m2";
		else
