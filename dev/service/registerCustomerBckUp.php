<?php
	...
	{
		$message = "Bonjour " . $firstname . " " . $name . ",<br/>";
		...
		$url = "<a href='" . $url . "'> http://backoffice.youfid.fr/activateAccount.php </a>";
		...
		$message .= "Pensez à télécharger l'application si vous ne l'avez pas encore fait :<br/>"
						. "<a href='http://android.youfid.fr'> http://android.youfid.fr </a><br/>"
						. "<a href='http://ios.youfid.fr'> http://ios.youfid.fr </a><br/>";
	...
	function get_email_physique_message($qr_code)
	{
		...
		$message .= "Pour finaliser la création de votre compte, veuillez cliquer sur le lient suivant:" . $url . "<br/>";
		//$message .= "Pour finaliser la création de votre compte, veuillez cliquer sur le lient suivant: http://backoffice.youfid.fr/activateAccount.php?qr_code=" . $qr_code;
		...
		else
		{
			...
			if (checkEmail($jsonArray->email) == TRUE)
			{
				...
				else
				{
					...
					mail_youfid($jsonArray->email, "Votre inscription YouFid", $email_content);
				}
			}
			else {
				$error = true;
	...
	function checkFb_id($fb_id)
	{
		...
		$sqlGetCustomer = "SELECT * FROM $tbl_name WHERE `fb_id`='"
